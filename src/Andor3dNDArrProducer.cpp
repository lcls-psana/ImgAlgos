...
// This Class's Header --
//-----------------------
#include "ImgAlgos/Andor3dNDArrProducer.h"
...
PSANA_MODULE_FACTORY(Andor3dNDArrProducer)
...
// Constructors --
//----------------
Andor3dNDArrProducer::Andor3dNDArrProducer (const std::string& name)
  : Module(name)
  , m_str_src()
  , m_key_in()
  , m_key_out()
  ...
  , m_print_bits()
{
    m_str_src    = configSrc("source",     "DetInfo(:Andor3d)");  // DetInfo(Camp.0:Andor3d.1)
    m_key_in     = configStr("key_in",     "");
    m_key_out    = configStr("key_out",    "Andor3d-ndarr");
...
// Destructor --
//--------------
Andor3dNDArrProducer::~Andor3dNDArrProducer ()
{
}
...
void 
Andor3dNDArrProducer::beginJob(Event& evt, Env& env)
{
...
void 
Andor3dNDArrProducer::beginCalibCycle(Event& evt, Env& env)
{
  ...
  if (config1.get()) {    
    if( m_print_bits & 4 ) {
      WithMsgLog(name(), info, str) {
        str << "Andor3d::ConfigV1:";
        str << "\n  width              = " << config1->width              ();
        str << "\n  height             = " << config1->height             ();
        str << "\n  numSensors         = " << config1->numSensors         ();
        str << "\n  orgX               = " << config1->orgX               ();
        str << "\n  orgY               = " << config1->orgY               ();
        str << "\n  binX               = " << config1->binX               ();
        str << "\n  binY               = " << config1->binY               ();
...
void 
Andor3dNDArrProducer::event(Event& evt, Env& env)
{
  procEvent(evt, env);
}
...
void 
Andor3dNDArrProducer::procEvent(Event& evt, Env& env)
{  
...
void 
Andor3dNDArrProducer::checkTypeImplementation()
{  
...
void 
Andor3dNDArrProducer::printInputParameters()
{
